...
def compute_sector_neutral_momentum(panel: pd.DataFrame, sector_data: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Compute Alpha 34 as stock 126-day momentum minus GICS sector momentum."""
    merged = panel.merge(
        sector_data[["date", "sector", "sector_price", "sector_return", "sector_momentum_126"]],
        on=["date", "sector"],
        how="left",
    )
    merged[ALPHA_34] = pd.to_numeric(merged[ALPHA_30], errors="coerce") - pd.to_numeric(
        merged["sector_momentum_126"], errors="coerce"
    )
    helper = merged[["date", "ticker", "sector", "sector_price", "sector_return", "sector_momentum_126"]].copy()
    merged = merged.drop(columns=["sector_price", "sector_return", "sector_momentum_126"])
...
def run_formula_spot_checks(
    ...
    sample = sample.merge(
        sector_helper[["date", "ticker", "sector_momentum_126"]],
        on=["date", "ticker"],
        how="left",
    )
    sample["expected_34"] = pd.to_numeric(sample[ALPHA_30], errors="coerce") - pd.to_numeric(
        sample["sector_momentum_126"], errors="coerce"
    )
    ...
    results["alpha_34"] = compare(ALPHA_34, "expected_34")
...
def build_enriched_panel(
    ...
    sector_result = load_sector_indices(sector_dir)
    ...
    if sector_result.loaded_count != len(SECTOR_FILE_MAP):
        qa.failures.append(f"sector files loaded count is {sector_result.loaded_count}, expected {len(SECTOR_FILE_MAP)}")
    if sector_result.staples_discretionary_identical:
        qa.failures.append("Consumer Staples and Consumer Discretionary sector files appear identical")
    ...
    panel, sector_helper = compute_sector_neutral_momentum(panel, sector_result.frame)
...
def print_enrichment_qa(panel: pd.DataFrame, context: dict[str, Any], qa: EnrichmentQA, output_path: Path, csv_path: Path | None) -> None:
    ...
    for col in TARGET_ALPHA_COLUMNS:
        ...
        if col == ALPHA_34 and sector_helper is not None:
            success_rate = float(sector_helper["sector_price"].notna().mean())
            print(f"    sector merge success rate: {success_rate:.4f}")
